...
class PoolEmbedding(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        self.projection = pooling(output_size=(patch, patch*2))
...
class DCA(nn.Module):
    def __init__(self,
        ...
        self.bn_relu = nn.ModuleList([nn.Sequential(
            ...
            for feature in features])
        self.cba=CBAMLayer(channel=1536)
    ...
    def forward(self, raw):
        ...
        # print("4",x_out[0].shape)
        x_out = self.m_apply(x_out, self.bn_relu)
        # print("8",x_out[0].shape)
        return (*x_out,)
    ...
    def m_sum(self, x, y):
        # print("4.1",x[0].shape,y[0].shape)
        # print("4.2", x[1].shape, y[1].shape)
        # print("4.3", x[2].shape, y[2].shape)
        return [xi + xj for xi, xj in zip(x, y)]
...
#
#     output1, output2, output3 = dca_model((input1, input2, input3))
#
#     print("\nOutput Shapes:")
